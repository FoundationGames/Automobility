...
public class JsonEMCodecs {
    ...
    public static final Codec<PartPose> PART_POSE = RecordCodecBuilder.create((instance) ->
            instance.group(
                    Vector3f.CODEC.optionalFieldOf("origin", Vector3f.ZERO).forGetter(obj -> new Vector3f(obj.x, obj.y, obj.z)),
                    Vector3f.CODEC.optionalFieldOf("rotation", Vector3f.ZERO).forGetter(obj -> new Vector3f(obj.xRot, obj.yRot, obj.zRot))
            ).apply(instance, (origin, rot) -> PartPose.offsetAndRotation(origin.x(), origin.y(), origin.z(), rot.x(), rot.y(), rot.z()))
    );

    public static final Codec<CubeDeformation> CUBE_DEFORMATION = Vector3f.CODEC.xmap(
    ...
    private static CubeDefinition createCuboidData(Optional<String> name, Vector3f offset, Vector3f dimensions, CubeDeformation dilation, boolean mirror, UVPair uv, UVPair uvSize) {
        return CubeDefinitionAccess.automobility$create(name.orElse(null), uv.u(), uv.v(), offset.x(), offset.y(), offset.z(), dimensions.x(), dimensions.y(), dimensions.z(), dilation, mirror, uvSize.u(), uvSize.v());
    }
    ...
    public static final Codec<CubeDefinition> CUBE_DEFINITION = RecordCodecBuilder.create((instance) ->
            instance.group(
                    ...
                    Vector3f.CODEC.fieldOf("offset").forGetter(obj -> ((CubeDefinitionAccess)(Object)obj).automobility$offset()),
                    Vector3f.CODEC.fieldOf("dimensions").forGetter(obj -> ((CubeDefinitionAccess)(Object)obj).automobility$dimensions()),
